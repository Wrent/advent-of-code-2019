...
fun main() {
    ...
    INPUT14.split("\n")
        .forEach { reaction ->
            ...
            val from = inputs.split(", ")
                .map { parseChemical(it) }
                .map { (quantity, name) ->
                    chemicals.putIfAbsent(name, Chemical(name))
                    Pair(quantity, chemicals[name]!!)
                }
            val (quantity, name) = parseChemical(output)
            chemicals.put(name, Chemical(name, quantity, from))
        }
...
const val TEST143 = """157 ORE => 5 NZVS
165 ORE => 6 DCFZ
...
const val TEST144 = """2 VPVL, 7 FWMGM, 2 CXFTF, 11 MNCFX => 1 STKFG
...
176 ORE => 6 VJHF"""
const val TEST145 = """171 ORE => 8 CNZTR
...
const val INPUT14 = """1 JNDQ, 11 PHNC => 7 LBJSB
1 BFKR => 9 VGJG
11 VLXQL => 5 KSLFD
117 ORE => 6 DMSLX
