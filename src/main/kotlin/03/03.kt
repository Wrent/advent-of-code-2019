...
fun main() {
    val (first, second) = INPUT3.split("\n").map { init(it) }
    val intersections = first.keys.intersect(second.keys)
    val closestIntersectionDistance = intersections.map { Math.abs(it.x) + Math.abs(it.y) }.sorted().first()

    println("first result")
    println(closestIntersectionDistance)

    println("second result")
    val intersectionWithSmallestNumberOfSteps = intersections.map { first[it]!! + second[it]!! }.sorted().first()
    println(intersectionWithSmallestNumberOfSteps)
}

fun init(wire: String): Map<Coord, Int> {
    val coords = mutableListOf<CoordWithSteps>()
    wire.split(",").forEach { processInstruction(it, coords) }
    val map = mutableMapOf<Coord, Int>()
    coords.forEach{ map.putIfAbsent(it.coord, it.steps)}
    return map
}

fun processInstruction(instruction: String, coords: MutableList<CoordWithSteps>) {
    val direction = instruction[0]
    val steps = instruction.substring(1).toInt()

    val last = coords.lastOrNull() ?: CoordWithSteps(Coord(0, 0), 0)
    when (direction) {
        'R' -> {
            addCoords(steps, coords, last) { i, coord ->
                Coord(coord.x + i, coord.y)
            }
        }
        'L' -> {
            addCoords(steps, coords, last) { i, coord ->
...
private fun addCoords(steps: Int, coords: MutableList<CoordWithSteps>, last: CoordWithSteps, newCoordFn: (Int, Coord) -> Coord) {
    var lastSteps = last.steps
    for (i in 1..steps) {
        coords.add(CoordWithSteps(newCoordFn(i, last.coord), ++lastSteps))
    }
}

data class CoordWithSteps(val coord: Coord, val steps: Int) {
    override fun equals(other: Any?): Boolean {
        if (this === other) return true
        if (javaClass != other?.javaClass) return false

        other as CoordWithSteps
